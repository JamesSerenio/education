...
interface ScoreWithQuizzes {
  id: string;
  total_score: number | null; // Changed from score to total_score for consistency
...
const Arithmetic_Radar: React.FC = () => {
  ...
  const fetchRadarData = async () => {
    ...
    try {
      ...
      const { data: allScores, error: scoresError } = await supabase
        .from("scores")
        .select(`id, total_score, time_taken, created_at, quiz_id, quizzes!inner(id, category, subject)`) // Changed to total_score and inner join for consistency
        .eq("user_id", user.id)
        .eq("quizzes.subject", "Arithmetic Sequence") // Focus on this subject
  ...
  useEffect(() => {
    ...
    gradient.addColorStop(0, "rgba(101, 163, 13, 0.35)"); // Green theme for Arithmetic
    gradient.addColorStop(1, "rgba(234, 179, 8, 0.35)"); // Yellow theme for Arithmetic
        ...
        datasets: [
          {
            ...
            borderColor: "#65a30d", // Green border
            borderWidth: 3,
            pointBackgroundColor: "#eab308", // Yellow points
